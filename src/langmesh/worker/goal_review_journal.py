"""Product adapter that records core goal-review events as the daemon's A2A transcript."""
...
class DaemonGoalReviewJournal:
    """Translate the library's typed review stream into the product's durable and live lanes."""

    def __init__(self, turn_store: Any, model_identifier: Callable[[], str]) -> None:
    ...
    async def open(self, context: GoalReviewContext) -> None:
        ...
        def emit_delta(channel: str, block_id: str, text: str) -> None:
            ...
            state.event_bus.publish_delta(context.review_id, channel, block_id, text)
...
__all__ = ["DaemonGoalReviewJournal"]
