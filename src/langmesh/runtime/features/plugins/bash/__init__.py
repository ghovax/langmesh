"""The bash plugin: running a shell command inside the session's confinement.

The library core never names bash: the tool, its description, and its event-rich handler are
this plugin's, contributed through the feature seam when the host composes it. A bare library
embedding has no shell surface at all.
"""
...
class Bash(Feature):
    ...
    def contribute_tools(self) -> list:
        """Bash rides for a profile that declared it. The schema map the host builds for name
        resolution has no context yet, so an unattached plugin still offers its tool."""
