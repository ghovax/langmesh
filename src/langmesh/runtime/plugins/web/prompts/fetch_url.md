...
- Use this for a URL you already know; use `search_web` to find one. Returns page text; handles JavaScript-rendered pages and common anti-bot walls through rendering fallbacks. A very large response is truncated inline and carries a `content_artifact` that the embedding can retrieve through its artifact store.
