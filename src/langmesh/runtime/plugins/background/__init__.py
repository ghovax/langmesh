...
class BackgroundState:
    """Background results already represented by a durable conversation checkpoint."""

    delivered_job_ids: tuple[str, ...] = ()

    @classmethod
    def from_data(cls, value: object) -> "BackgroundState":
        if isinstance(value, cls):
            return value
        if not isinstance(value, Mapping):
            return cls()
        identifiers = value.get("delivered_job_ids", ())
        return cls(
...
class BackgroundJobsFeature(Feature):
    ...
    def __init__(self, *, store: Any = None) -> None:
        self._store = store
        self._checkpointed_deliveries: set[str] = set()
    ...
    def attach(self, context: PluginContext, host: PluginHost) -> None:
        ...
        self._runner = BackgroundJobs(
            ...
            store=self._store,
            note_state_changed=host.bookkeeping.note_state_changed,
        )
    ...
    def snapshot(self) -> BackgroundState:
        return BackgroundState(
            tuple(sorted(self._checkpointed_deliveries | set(self._runner.pending_deliveries)))
        )

    def restore(self, state: object) -> None:
        self._checkpointed_deliveries = set(BackgroundState.from_data(state).delivered_job_ids)
    ...
    def inject_stored_result(
        ...
        self._runner.stage_delivery(identifier)
        if identifier in self._checkpointed_deliveries or self._conversation_has(identifier):
            return
    ...
    def _conversation_has(self, identifier: str) -> bool:
        for message in self._host.conversation.messages:
            marks = getattr(message, "additional_kwargs", {}) or {}
            metadata = marks.get("background_result")
            if isinstance(metadata, Mapping) and metadata.get("background_job_id") == identifier:
                return True
        return False
