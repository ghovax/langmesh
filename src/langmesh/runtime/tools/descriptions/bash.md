...
Python is your Swiss-army knife and there is no task it is out of scope for: use it for parsing, restructuring, querying, analyzing, generating, and orchestrating work of any shape, and compose it freely with other commands — pipe its output through further shell tools, wrap several shell commands inside a Python heredoc, or have one call do many independent things at once. Before assuming Python or a Python tool is unavailable, check for `uv`: prefer `uv run python` for project-aware scripts, `uvx <tool>` for an ephemeral CLI, and `uv run --with <package> python` or `uvx --with <package> <tool>` when a task needs packages on the fly; fall back to bare `python` only when `uv` is unavailable. Prefer Python over shell string surgery whenever it would be clearer, safer, or faster, and do not ration it. Semble is a key tool for locating code and mechanisms semantically; use it freely, treat it as cheap enough to run repeatedly with a fresh disposable index, and combine it with exact searches to verify and narrow results.
