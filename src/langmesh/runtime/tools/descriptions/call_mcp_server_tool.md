...
- Treat safety as in `bash`: `access_request` with `mutates: false` to inspect,
  `mutates: true` to change.
- An MCP server runs outside this machine's confinement: a call that changes anything is
  put to whoever decides for this session unless a rule already names it.
