...
async def _reload_mcp() -> None:
    ...
    async with state.configuration_lock:
        state.application_configuration.mcp = load_configuration(seed=False).mcp
        # Re-fold the provisioned Composio server in, so a live edit does not drop its tools.
        state.application_configuration.mcp.servers.update(state.composio_servers)
        enabled = state.application_configuration.mcp.enabled_servers()
...
async def _ensure_mcp_servers_for(working_directory: str) -> None:
    ...
    async with state.configuration_lock:
        ...
        new_servers = {
            ...
            if state.application_configuration.mcp.servers.get(name) != configuration
        }
        if not new_servers:
            return
        state.application_configuration.mcp.servers.update(new_servers)
        enabled = state.application_configuration.mcp.enabled_servers()
        if state.mcp_server_manager is None:
            if enabled:
                state.mcp_server_manager = MCPServerManager(enabled)
