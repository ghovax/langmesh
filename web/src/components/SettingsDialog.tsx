...
import { ChatGPTAuthControl } from "./ChatgptAuth";
import { CursorAuthControl } from "./CursorAuth";
import { WorkspaceLocationsPanel } from "./WorkspaceLocations";
...
import { useSchemaSettingsPage } from "./SettingsSchemaPages";
import {
  AgentSelectControl,
...
export function SettingsDialog({
  ...
  useEffect(() => {
    ...
    fetchSettings()
      ...
      .catch((caught) =>
        reportError({ component: "settings", operation: "read the settings" }, caught),
      );
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    return subscribeEvents((event) => {
      ...
      fetchSettings()
        ...
        .catch((caught) =>
          reportError({ component: "settings", operation: "read the agent configuration" }, caught),
        );
    });
  }, [open]);
  ...
  useEffect(() => {
    ...
    return subscribeEvents((event) => {
      ...
      fetchAgentConfiguration(agent, directory)
        ...
        .catch((caught) =>
          reportError({ component: "settings", operation: "read the permission state" }, caught),
        );
    });
  }, [open]);
