...
- A GitHub mention is the library in a GitHub Action: `@langmesh` on an issue or pull request starts `langmesh.github.mention` with `LANGMESH_MODEL` as `provider/model`. There is no daemon. On a new issue the agent creates `langmesh/<name>-<4 hex>` itself and the job opens a draft PR; a pull-request mention pushes to that PR's branch. See [GitHub mentions](../user/github.md).
