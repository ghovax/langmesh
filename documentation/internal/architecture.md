...
**The cache is asked for by name.** A provider does not keep one global store; it routes a lookup by hashing the head of the prefix together with a key, so requests that share a key land where their prefix is. Every provider that accepts one is sent `prompt_cache_key` set to the session id, because one session is one conversation is one prefix. The initial bound tool schema stays fixed through ordinary work. A live tool grant is an explicit capability mutation: it rebinds the real provider schema, records a tools-segment divergence on the next call, and establishes the new reusable prefix thereafter. Auxiliary lanes keep independent local baselines so their requests never replace the ordinary conversation's comparison chain. Claude additionally gets rolling explicit `cache_control` breakpoints: the two newest attempted tails are retained per lane, both are marked again when reachable, and the new tail becomes the next candidate. That fallback is what preserves a warm breakpoint when a response or connection disappears before the provider confirms the newest write.
...
| `refresh_prompt()` or cache-stable feature metadata refresh | First instructions segment is rebuilt explicitly | Intentional divergence | Following calls reuse the refreshed prefix. |
