...
- A GitHub mention is the library in a GitHub Action: `@langmesh[bot]` on an issue or pull request starts `langmesh.github.mention` with `LANGMESH_MODEL` as `provider/model`. There is no daemon. The job acknowledges immediately, links the live Action log on that comment, and updates it; failures go to the Action log, not the thread. On an issue the agent reuses a branch that already is this work when one exists, otherwise creates `langmesh/<slug>-<code>` itself, commits, and the job opens a draft PR; a pull-request mention pushes to that PR's branch. The workflow caches `.github/langmesh` per thread, including provider cache state, and the mention system prompt is byte-stable so follow-up jobs reuse the prefix. See [GitHub mentions](../user/github.md).
