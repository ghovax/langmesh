...
- A GitHub mention is handled by the installation-level App service: `@<app-slug>[bot]` on an issue or pull request, or a reply to the bot, starts `langmesh.github.mention`. The service receives signed webhooks, mints a repository-scoped installation token, loads provider/model credentials encrypted by installation, and keeps workspaces and session checkpoints outside the repository. It acknowledges and updates the same thread comment, commits as the installed App, and opens draft pull requests for issue work. See [Universal GitHub App](../user/github.md).
