...
- A GitHub mention is the library in a GitHub Action: `@langmesh` on an issue or pull request starts `langmesh.github.mention` with `LANGMESH_MODEL` as `provider/model`. There is no daemon. An issue with file edits opens a draft PR on `langmesh/<name>-<4 hex>`; a pull-request mention pushes to that PR's branch. See [GitHub mentions](../user/github.md).
