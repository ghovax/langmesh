...
- **No hard cap bounds an open goal.** While a goal stays open and unmarked the session keeps being reminded to reach it — reaching it, or the person stopping it, is what ends it. Only the tracked-task allowance is a hard bound. The goal is visible in the app above the composer, with a control that calls it off outright, which clears it for the person.
