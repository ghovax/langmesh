...
- Its final action is a structured `submit_goal_review` call naming the requirements it could not prove, whether the formal contract is complete, whether the goal is unmet, satisfied, or blocked, and the exact next message when work remains. The tool accepts the verdict on any review, gated only by the reviewer's own evidence. A missing verdict is retried within one total attempt and time budget; exhaustion fails that review instead of opening an endless reminder loop. Cancelling the parent session cancels the review. The review confirms an agent-marked `satisfied` or `blocked` only on evidence, and overrides a mark the work does not support, sending the goal back to reasoned work with a message. `parked` and `cleared` are administrative marks applied directly, and an open goal a turn ends on without a mark is re-opened with a light continuation reminder rather than a review.
