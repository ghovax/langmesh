...
- One image, two entry points, chosen by the first argument: `langmesh` (the CLI: `serve`, `mail`, and the deployment-specific `github` service) and `langmeshd` (the daemon that hosts sessions). A bare launch lands in the CLI.
...
For the complete procedure with expected output, see [Installation](../user/installation.md#build-checklist).
...
| `cli/`        | The `langmesh serve`, `langmesh mail` / `langmesh mail check`, and deployment-specific `langmesh github` commands |
