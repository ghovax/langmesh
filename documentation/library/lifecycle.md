...
The `providers` mapping is split into explicit provider credentials and endpoint values; the caller's mapping is never mutated. Environment variables still take precedence. Account-backed providers (`chatgpt`, `cursor`) use the replaceable `Credentials` port in `SessionComponents`.
