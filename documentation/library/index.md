...
- [Universal GitHub App](../user/github.md) handles GitHub events through signed installation webhooks. Provider/model settings, the delivery queue, and session checkpoints use an external database, while each worker uses a disposable checkout. An issue with file edits opens a draft PR; a pull-request event updates that PR.
