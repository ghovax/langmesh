...
- [GitHub mentions](../user/github.md) is the library running in a GitHub Action when someone writes `@langmesh`. `LANGMESH_MODEL` is `provider/model`. The posted comment comes from `submit_github_comment`; real prompts are markdown under `src/langmesh/github/prompts/`.
