...
- [Universal GitHub App](../user/github.md) handles `@<app-slug>[bot]` comments through signed installation webhooks. Provider/model settings are encrypted per installation, while workspaces and checkpoints stay outside repositories. An issue with file edits opens a draft PR; a pull-request mention updates that PR.
