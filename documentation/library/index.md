...
- [GitHub mentions](../user/github.md) is the library running in a GitHub Action when someone writes `@langmesh` on an issue or a pull request. An issue with file edits opens a draft PR; a pull-request mention updates that PR. `LANGMESH_MODEL` is `provider/model`. The posted comment comes from `submit_github_comment`; real prompts are markdown under `src/langmesh/github/prompts/`.
