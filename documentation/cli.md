...
This is the **only** place a session's agent and its directory are set, and nothing changes either afterwards. Its permission mode is the one piece that moves: it can be loosened once a flow has earned trust, or tightened, and the change reaches the turn already in flight. What it can never do is pass the clamp the session was created under — its parent's mode and its agent profile's ceiling both still bind, and tightening a session tightens everything it created. That ceiling is what makes a session's authority something you can reason about, and a later call cannot raise it.
