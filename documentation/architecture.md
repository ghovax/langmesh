...
- **MCP servers**, through the session's own connections. Stateful connections and stdio subprocesses do not cross a process boundary, so a session connects to its own servers rather than sharing the daemon's connections.

5. Results take two deliberately separate lanes. Every model text/reasoning delta is published synchronously to the daemon's in-process bounded event bus and reaches attached SSE clients without a timer, task allocation or database write; the browser coalesces only at its next paint. At semantic boundaries the accumulated content block is written once to `history.sqlite`. An attachment first establishes an atomic durable/live high-water cut, then streams complete compacted turns continuously from newest to oldest while live events preempt history work. The browser prepends each turn once with stable identities, so opening cost is independent of conversation length, replay is linear rather than repeatedly rebuilding the prefix, and half a tool lifecycle can never cross a history frame. Slow clients receive an explicit resync instead of consuming unbounded memory or silently losing text. Model-facing tool results follow the same separation: the required contiguous `ToolMessage` block contains only execution data, timing and lifecycle metadata live in LangChain artifacts that providers do not receive, and explanatory guidance is appended afterward as a distinct hidden reminder. Provider adapters preserve that reminder as an instruction (`system` for LiteLLM, `developer` for Codex and an instruction transcript block for Cursor) rather than pretending the user said it. No prior message is rewritten, so reviewer branches and later turns retain the exact provider-cache prefix.
...
Tracked tasks have their own continuation obligation and allowance. At the completed-turn boundary, the session derives one plan containing the independent `goal` and `tasks` reasons. One owned workflow drains those plans under the session's turn lock: it reviews the goal first, then compacts the hidden unfinished-task audit into the review's continuation message, or opens a task-only reminder turn when the review produces no next message. This makes competing automatic turns structurally impossible while allowing either mechanism to operate without the other. Task status remains agent-owned; dependency readiness is derived separately, so completing a dependency cannot silently reopen an explicitly blocked task. Stops, pending background jobs, blocked-only or dependency-waiting lists, and exhausted allowances suppress automatic work; a new user turn restores both allowances.
...
**The cache is asked for by name.** A provider does not keep one global store; it routes a lookup by hashing the head of the prefix _together with_ a key, so requests that share a key land where their prefix is. Every provider is sent `prompt_cache_key` set to the session id — one session is one conversation is one prefix. Permission review, goal review and compaction append their private instructions only after the main conversation prefix and retain the exact same ordered model-tool schema, including non-executable internal verdict schemas carried by ordinary turns solely to keep that wire prefix invariant. Their local trace branches start from the main trace while continuing to use the same provider key, so auxiliary calls can advance independently without invalidating the ordinary turn that follows. Claude additionally gets explicit `cache_control` breakpoints, two at the front over the tools and prompt and two at the moving end, since Anthropic caches nothing unless asked.
