...
This is the library in a short-lived job, not the daemon. The workflow lives at `.github/workflows/langmesh.yml`; the session is composed in `langmesh.github.mention`. The job posts an acknowledgement after checkout — from `acknowledgement.md` and `working_comment.md` — and before the venv. That comment carries a link to the Action, whose log streams live in the GitHub UI. While the turn runs, the job refreshes a short “working for N minutes” note on the same comment; it does not invent the direction of the work. The agent writes that same comment through `submit_github_comment`: a short progress note when the direction changes (`kind` `progress`), then the reply (`kind` `reply`). Model prose is not posted. Failures are written to the Action log with the same logger the daemon uses; the thread only gets a short, user-facing note and a link to that log. Real prompts (the system prompt, the turn, the tool description, the missing-call reminder, the invalid-model note, the acknowledgement, the working comment) are markdown templates under `src/langmesh/github/prompts/`. Short strings the Action itself writes (a commit message, a pull-request title, `Done.`) stay in code.
...
- The job posts an acknowledgement after checkout, from the mention prompt files, before installing the venv. The acknowledgement links the Action so the log can be watched live, and the job refreshes how long it has been working. The agent **edits that comment** through `submit_github_comment` when the direction of the work changes, then overwrites it with the finished reply or a short failure note. It does not add a second comment for the result.
