...
On Render, set the service's `maxShutdownDelaySeconds` to `300`. During a shutdown, the worker stops claiming new deliveries and lets the active delivery finish within `shutdown_grace_seconds`. If the platform ends that window first, the active delivery is returned to the database queue immediately and resumes from its durable checkpoint after restart.
