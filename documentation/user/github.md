...
### Render lifecycle and shutdown protection

Render does not provide an in-process "about to suspend" API or a reliable shutdown timestamp. For a zero-downtime deploy or restart, Render routes traffic to the replacement instance, waits 60 seconds, and then sends `SIGTERM` to the old process. The default shutdown delay is 30 seconds; paid services can set `maxShutdownDelaySeconds` up to 300 seconds. Render sends `SIGKILL` when that delay expires. The FastAPI lifespan receives this normal shutdown, marks the processor as draining, returns `503 Retry-After: 5` for new webhook deliveries, and stops the queue claim loop. An active delivery is allowed to finish first. If the grace window expires, the worker interrupts the active session, saves the LangMesh checkpoint (conversation, feature state, local request-cache baselines, and provider-native cache snapshot), returns the processing row to the external queue, and exits.

A provider's remote prompt cache has no flush or close operation exposed by the service or by Render. The durable checkpoint preserves the exact request baseline needed to diagnose and reuse the local cache state after recovery; it cannot force a third-party provider to retain an entry or make an already-sent remote cache write atomic with process shutdown. The temporary checkout and artifact directory are also not recovery storage. Keep the database and checkpoints outside the Render filesystem.

The service exposes lifecycle counters at `/metrics` and writes the same decisions to the Render log. Watch `langmesh_github_shutdown_requests_total`, `langmesh_github_shutdown_forced_total`, `langmesh_github_shutdown_checkpoint_attempts_total`, `langmesh_github_shutdown_checkpoint_failures_total`, `langmesh_github_shutdown_requeues_total`, and `langmesh_github_delivery_recoveries_total`. `langmesh_github_shutdown_active_delivery` and `langmesh_github_shutdown_requested` are gauges. These counters are process-local and reset after a restart; the durable delivery status and checkpoint are authoritative.

This protection is best effort for interrupted restarts. A Free web service can restart at any time, its local filesystem is ephemeral, and idle spin-down or account-level suspension does not promise advance notice to the process. A process killed with `SIGKILL`, machine failure, provider/network interruption, or suspension before `SIGTERM` cannot run cleanup. In those cases the external queue's stale-claim recovery requeues the delivery and the next worker resumes from the last committed checkpoint. Do not use a Free Render service or a local filesystem for sole durable storage. The keep-awake workflow reduces idle spin-down but cannot guarantee delivery of a warning or prevent an unannounced restart.
