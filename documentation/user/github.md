...
This is the library in a short-lived job, not the daemon. The workflow lives at `.github/workflows/langmesh.yml`; the session is composed in `langmesh.github.mention`. The job posts an acknowledgement as soon as it has a GitHub token — before checkout and Python. The agent writes that same comment through `submit_github_comment`: a short progress note when the direction changes (`done` false), then the finished reply (`done` true). Model prose is not posted. Failures are written to the Action log with the same logger the daemon uses; the thread only gets a short, user-facing note and a link to that log. Real prompts (the system prompt, the turn, the tool description, the missing-call reminder, the invalid-model note) are markdown templates under `src/langmesh/github/prompts/`. Short strings the Action itself writes (the acknowledgement, a commit message, a pull-request title, `Done.`) stay in code.
...
- The GitHub comment is `submit_github_comment`. Progress notes use `done` false and keep the turn open. The finish uses `done` true. If the turn ends without that finished call, the session reminds the model until it submits. An empty finished comment is posted as `Done.`
- Commits use a sentence subject taken from the finished comment, in the same style as the rest of this repository, not a `langmesh: …` prefix.
