...
- A second comment that **tags the bot** while a mention job is still running is steered into that live session at the next model opening. The queued job for that comment restores the session artifact, sees the comment was already ingested, and skips. Replies that do not tag the bot still wait and start their own turn.
