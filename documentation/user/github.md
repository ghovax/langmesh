...
- `@langmesh` on an **issue** does the work. If files will change, the agent creates `langmesh/<name>-<4 hex>` itself (`git checkout -b`) — it chooses the name, the job assigns the suffix. The Action then commits, pushes, and opens a **draft** pull request. A later mention on that issue continues that draft. The Action never marks it ready — a person does that.
