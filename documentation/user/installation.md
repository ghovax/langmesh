...
An issue or pull-request comment that mentions `@langmesh[bot]`, or a reply to the bot, can run the library in a GitHub Action. Cite `@langmesh[bot]`, not `@langmesh`: the [`@langmesh`](https://github.com/langmesh) user already exists and would be notified. On an issue it can open a draft pull request; on a pull request it updates that branch. The model is the GitHub agent profile; the key is the secret file `github.api_key` (Actions secret `mention_api_key` on a hosted runner). To have GitHub suggest a bot, install a GitHub App under an available name — `LangMesh` is reserved — put `app_id` in `.github/langmesh.yaml`, and the PEM in the secret file `github.app.private_key` (Actions secret `mention_app_private_key` on a hosted runner). See [GitHub mentions](github.md).
