...
- When a session needs permission, the turn stops and an overlay appears, saying what the tool wants and why. Allow or deny, and the turn goes on.
- There is no standing "always allow" prompt, and a session's permission mode can change at any point from the chip under the composer, including mid-turn. See [Permission modes](configuration.md#permission-modes). (Setting a session to `allow` mode skips the gate entirely, but the operating-system confinement still applies.)
...
- A locally queued message appears immediately with its stable client id, marked queued or sending until the daemon accepts that exact id. Live streaming and replay reduce the same durable event.
...
- A **workspace** is a project: it holds your conversations and a set of **environments** (locations). An environment says where a session's work happens. It can be a directory on this machine, or one on an SSH host. Add one under **Settings, then Environments**, or with the **New workspace** dialog in the sidebar.
...
- Adding or editing an environment reaches sessions already running in that workspace: they pick up its locations on their next turn, not only after a restart.
...
The tool reads the accessibility tree and the page's structure, not screenshots. See [Screen control](agent-system.md#screen-control-control_screen) for what it can and cannot do.

## A daemon somewhere else

The app is a client, so the daemon it talks to need not be the one on this machine. That is a different question from where a session's **tools** run.
...
- The same interface in a browser, and a phone over the pairing door: [`langmesh serve`](installation.md#the-langmesh-command).
